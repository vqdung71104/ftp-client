...
void send_upload_command(int sockfd, char *recv_buf)
{
    ...
    // Receive response: +OK <filesize> or ERR
    int len = recv_line(sockfd, buffer, BUFF_SIZE);
    if (len <= 0)
    {
        printf("Server disconnected or error.\n");
        return;
    ...
    while (total_received < filesize)
    {
        ...
        int to_recv = (remaining < BUFF_SIZE) ? remaining : BUFF_SIZE;
    ...
    {
        printf("Error: Received %ld bytes but expected %ld bytes.\n", total_received, filesize);
        remove(filename);
    }
}
...
void send_download_command(int sockfd, char *recv_buf)
{
    ...
    // Craft payload: DNLD <filename> <filesize>
    char command[1280];
    snprintf(command, sizeof(command), "DNLD %s %ld\r\n", filename, file_size);

    if (send(sockfd, command, strlen(command), 0) < 0)
    {
        perror("Send() error when sending DNLD command");
        fclose(fp);
        return;
    }
    ...
    while((bytes_read = fread(buffer, 1, BUFF_SIZE, fp)) > 0){
        if(send(sockfd, buffer, bytes_read, 0) < 0){
            perror("send() error while downloading file");
            break;
        }
